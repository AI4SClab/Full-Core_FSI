...
import tracemalloc,time, os, sys,heapq
...
def update_fulid_velocity(nodes, nodef, M, R, s_dict):
    """处理相同层的中间节点"""
    ...
    for i in range(M):
        ...
        for fluid_point in fluid_points:
            ...
            for solid_point in solid_points:
                ...
                # 使用堆存储距离和流体点压力值
                heapq.heappush(min_heap, (dist_squared, solid_point))

            # 从堆中取出距离最小的点
            nearest_dist, solid_point = heapq.heappop(min_heap)
            if nearest_dist == 0:
                fluid_point.vx = solid_point.vx # 更新固体点的压力值
                fluid_point.vz =solid_point.vy
                fluid_point.vz = solid_point.vz
            else:
                # 找到最近固体点关联的四边形
                s_faceL = solid_point.neighborfaces
                ...
                for [sid1, sid2, sid3, sid4] in s_faceL:
                    ...
                    elif is_point_on_edge(fluid_point, sn1, sn2):
                        # 如果在边上，进行线性插值
                        vx,vy,vz = linear_interpolation_velocity(fluid_point, sn1, sn2)
                        fluid_point.vx = vx
                        fluid_point.vy = vy
                        fluid_point.vz = vz
                        # print(f"1距离非0={mindist}，在关联四边形{[sn1, sn2, sn3, sn4]}的边{[sn1, sn2]}上........压力值={fulid_node}")
                        break
                    elif is_point_on_edge(fluid_point, sn2, sn3):
                        vx,vy,vz = linear_interpolation_velocity(fluid_point, sn2, sn3)
                        fluid_point.vx = vx
                        fluid_point.vy = vy
                        fluid_point.vz = vz
                        # print(f"2距离非0={mindist}，在关联四边形{[sn1, sn2, sn3, sn4]}的边{[sn2, sn3]}上........压力值={fulid_node}")
                        break
                    elif is_point_on_edge(fluid_point, sn3, sn4):
                        vx,vy,vz = linear_interpolation_velocity(fluid_point, sn3, sn4)
                        fluid_point.vx = vx
                        fluid_point.vy = vy
                        fluid_point.vz = vz
                        # print(f"3距离非0={mindist}，在关联四边形{[sn1, sn2, sn3, sn4]}的边{[sn3, sn4]}上........压力值={fulid_node}")
                        break
                    elif is_point_on_edge(fluid_point, sn4, sn1):
                        vx,vy,vz = linear_interpolation_velocity(fluid_point, sn4, sn1)
                        fluid_point.vx = vx
                        fluid_point.vy = vy
                        fluid_point.vz = vz
...
def GetSolidNodeDic(SolidTree, N, M):
    s_dict = {}
    for layer_index in range(N):
        nodes = SolidTree.children[layer_index]
        for i in range(M):
            solid_points = nodes.children[i].points
            for solid_point in solid_points:
                s_dict[solid_point.ID] = solid_point
    return s_dict


def traverse_send_layers_StoF(FulidTree, SolidTree, N, M, R):
    """遍历两棵树，获取匹配的流体和固体树节点并进行插值计算，为流体网格点计算速度值"""
    s_dict = GetSolidNodeDic(SolidTree, N, M)
    newNodesL = []
    for layer_index in range(N):
        solid_node = SolidTree.children[layer_index]
        fluid_node = FulidTree.children[layer_index]

        new_fulid_node = update_fulid_velocity(solid_node, fluid_node, M, R, s_dict)
