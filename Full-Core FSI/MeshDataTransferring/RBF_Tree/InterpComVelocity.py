...
from saveandloadFuns import load_batch_from_disk, load_forest_index,save_batch_to_disk, save_forest_index
...
def update_fulid_velocity(nodef, nodes, M, R):
    """处理相同层的中间节点"""
    for i in range(M):
        ...
        # 更新流体结点的速度值
        for j in range(len(nodef.children[i].points)):
            nodef.children[i].points[j].vx = velocity0_matrix_f[j, 0]
            nodef.children[i].points[j].vy = velocity1_matrix_f[j, 0]
            nodef.children[i].points[j].vz = velocity2_matrix_f[j, 0]
...
def traverse_send_layers_StoF(FulidTree, SolidTree, N, M, R):
    ...
    for layer_index in range(N):
        ...
        new_fulid_node = update_fulid_velocity(fluid_node, solid_node, M, R)
        newNodesL.append(new_fulid_node)  # 收集结果

    return newNodesL

def interpolate_solid_to_fluid(num_batches):#, output_dir): 
